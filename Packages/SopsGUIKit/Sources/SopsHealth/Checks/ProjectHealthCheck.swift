    ...
    private func findings(for project: InspectedProject, idScope: String,
        ...
        guard FileManager.default.fileExists(atPath: configURL.path) else {
            ...
            return [scope.finding(
                ...
                remediation: Remediation(
                    ...
                    explanation: "Add a .sops.yaml file at the project root with a creation_rules entry naming the age public keys new files should be encrypted to. This app can propose one now: select this project, click the toolbar's New File button (or press ⌘N), add at least one recipient, then click Propose .sops.yaml — it shows the file before writing it, and writes it only once you confirm. Writing the file by hand is just as valid; see sops's own documentation for the file format.")), leak, permissions, unreadableByCreation]
        }
        ...
        do {
            ...
            return [scope.finding(
                ...
                remediation: Remediation(
                    explanation: "Fix the YAML syntax, then re-run this check.")), leak, permissions, unreadableByCreation]
        }
