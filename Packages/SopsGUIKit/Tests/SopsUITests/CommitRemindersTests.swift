...
struct CommitRemindersTests {
    ...
    func applyToFilesAsksForCommit() {
        let summary = LocalizedKey.projectAccessResultsSummary.text.lowercased()
        let note = LocalizedKey.projectAccessResultsCommitNote.text.lowercased()

        #expect((summary + " " + note).contains("commit"), Comment(rawValue: """
            ...
            though writing .sops.yaml does. Summary was: \
            \(LocalizedKey.projectAccessResultsSummary.text)
            """))
    }
}
